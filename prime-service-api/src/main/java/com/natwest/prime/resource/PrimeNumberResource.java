...
@Api( "An Api to return the list of prime numbers up to a top limit")
@RestController
@Validated
...
public class PrimeNumberResource {
    ...
    @Operation(
            tags = "Prime Service",
            summary = "Api returns list of all prime numbers starting from 2 for a given number with top limit"
    )
    ...
    public ResponseEntity<PrimeNumberResponse> getPrimes(@PathVariable("number") @Min(2) Long number,
                                                         @ApiParam(defaultValue = "default",
                                                                   allowableValues = "default, apache, bigint",
                                                                   type = "String",
                                                                   allowEmptyValue = true
                                                                    )
                                                         @RequestHeader(value = "serviceType", defaultValue = "default") String type) {
    ...
    private PrimeService getService(String type) {
        ...
        return primeServiceStream.orElseThrow(() -> new InvalidRequestException("Bad request - header parameter serviceType is invalid"));
